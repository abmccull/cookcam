...
export class SubscriptionService {
  ...
  async verifyIOSReceipt(receipt: string, userId: string): Promise<VerificationResult> {
    try {
      ...
      const subscription: SubscriptionInfo = {
        ...
        trialEnd: latestReceipt.is_trial_period === 'true' 
  ...
  async verifyAndroidPurchase(
    ...
    try {
      ...
      const subscription: SubscriptionInfo = {
        ...
        trialEnd: purchase.paymentState === 2 // Free trial
  ...
  async processGooglePlayWebhook(notification: Record<string, unknown>): Promise<void> {
    const subscriptionNotification = notification.subscriptionNotification as Record<string, unknown>;
  ...
  private async updateSubscriptionFromWebhook(
    ...
    if (!latest_receipt_info || !Array.isArray(latest_receipt_info) || latest_receipt_info.length === 0) {
      return;
    }
  ...
  private async verifyAndUpdateAndroidSubscription(
    purchaseToken: string
  ): Promise<void> {
