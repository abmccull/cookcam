...
export class AnalyticsService {
    ...
    try {
      ...
      const analyticsData = (recipes || []).map((recipe) => {
        const engagement_score =
          recipe.view_count * 1 + recipe.rating_count * 5 + recipe.rating_avg * 10;
  ...
  async getUserAnalytics(userId: string): Promise<UserAnalytics> {
    try {
      ...
      const uniqueDays = new Set(
        (activeDays || []).map((event) => new Date(event.timestamp).toDateString())
      );
      ...
      const mostScanned = Object.entries(ingredientCounts).sort(([, a], [, b]) => b - a)[0];
  ...
  async getAppStatistics(): Promise<any> {
    try {
      ...
      // Get various counts
      const [totalUsers, activeUsersToday, activeUsersWeek, totalScans, totalRecipes, totalPhotos] =
        await Promise.all([
