...
export class MonitoringService {
  ...
  private async checkExternalAPIs(): Promise<ServiceHealth> {
    ...
    const results = await Promise.all(apis.map((api) => api.check.call(this)));
  ...
  private async checkUSDA(): Promise<boolean> {
    try {
      ...
      const response = await fetch(
        `https://api.nal.usda.gov/fdc/v1/foods/search?query=apple&api_key=${process.env.USDA_API_KEY}`,
        {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
  ...
  async logMetrics(): Promise<void> {
    try {
      ...
      const { data: apiMetrics, error } = await client.rpc('get_performance_metrics');
