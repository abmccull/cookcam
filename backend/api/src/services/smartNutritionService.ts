...
function convertToGrams(quantity: number, unit: string, ingredientName: string): number {
  ...
  const baseConversions: { [key: string]: number } = {
    ...
    'whole': 150, 'item': 100, 'items': 100
  };

  // If it's a direct weight unit, use it
  if (baseConversions[unitLower] && !unitLower.includes('cup') && !unitLower.includes('ml') && !unitLower.includes('l')) {
  ...
  if (unitLower.includes('cup')) {
    ...
    if (ingredientLower.includes('milk') || ingredientLower.includes('water') || 
        ingredientLower.includes('broth') || ingredientLower.includes('stock') ||
        ingredientLower.includes('juice') || ingredientLower.includes('wine')) {
    ...
    if (ingredientLower.includes('flour') || ingredientLower.includes('powder') ||
        ingredientLower.includes('sugar') || ingredientLower.includes('salt')) {
      return quantity * 200; // 1 cup flour ≈ 200g
    }
    
    // Rice and grains
    if (ingredientLower.includes('rice') || ingredientLower.includes('quinoa') ||
        ingredientLower.includes('grain') || ingredientLower.includes('pasta')) {
      return quantity * 185; // 1 cup rice ≈ 185g
    }
    
    // Vegetables (chopped/diced)
    if (ingredientLower.includes('onion') || ingredientLower.includes('carrot') ||
        ingredientLower.includes('pepper') || ingredientLower.includes('vegetable')) {
...
function calculateStringSimilarity(str1: string, str2: string): number {
  ...
  for (const word1 of words1) {
    for (const word2 of words2) {
      if (word1 === word2 && word1.length > 2) { // Ignore short words like "of", "in"
        matches++;
        break;
      }
    }
...
function calculateEnhancedSimilarity(input: string, dbName: string): number {
  const normalizedInput = input.toLowerCase().replace(/[^a-z\s]/g, ' ').replace(/\s+/g, ' ').trim();
  ...
  try {
    ...
    for (const synonym of synonyms) {
      const { data: matches } = await supabase
        .from('ingredients')
        .select('name, calories_per_100g, protein_g_per_100g, carbs_g_per_100g, fat_g_per_100g, sodium_mg_per_100g')
        .ilike('name', `%${synonym}%`)
    ...
    console.log(`✅ Matched "${ingredientName}" → "${bestCandidate.name}" (${Math.round(bestCandidate.similarity * 100)}%)`);
