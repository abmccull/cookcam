...
async function checkDatabaseStatus() {
  try {
    ...
    console.log(
      `📈 USDA Coverage: ${(((usdaCount || 0) / (totalIngredients || 1)) * 100).toFixed(1)}%`
    );
    ...
    if (recentIngredients && recentIngredients.length > 0) {
      recentIngredients.forEach((ingredient, index) => {
        ...
        console.log(
          `   ${index + 1}. ${ingredient.name} (${ingredient.category}) ${usdaStatus} - ${date}`
        );
      });
    } else {
    ...
    if (fs.existsSync(progressFile)) {
      try {
        ...
        console.log('\n🔄 Seeding Progress:');
        console.log(
          `   • Processed: ${progressData.processedItems?.toLocaleString() || 0}/${progressData.totalItems?.toLocaleString() || 0}`
        );
        ...
        const percentage = (
          ((progressData.processedItems || 0) / (progressData.totalItems || 1)) *
          100
        ).toFixed(2);
