...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      total_boxes: total,
      rarity_distribution: {
        common: { count: rarityCount.common, percentage: (rarityCount.common / total * 100).toFixed(1) },
        uncommon: { count: rarityCount.uncommon, percentage: (rarityCount.uncommon / total * 100).toFixed(1) },
        rare: { count: rarityCount.rare, percentage: (rarityCount.rare / total * 100).toFixed(1) },
        legendary: { count: rarityCount.legendary, percentage: (rarityCount.legendary / total * 100).toFixed(1) }
      },
