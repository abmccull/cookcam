...
router.put('/profile', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    if (creator_bio !== undefined) {
      updateData.creator_bio = creator_bio;
    }
    if (creator_specialty !== undefined) {
      updateData.creator_specialty = creator_specialty;
    }
    if (creator_tier !== undefined) {
      updateData.creator_tier = creator_tier;
    }
    ...
    if (updateData.is_creator && !data.was_creator) {
      try {
        ...
        await supabase.from('creator_tiers').upsert([
          {
            user_id: userId,
            tier_level: updateData.creator_tier || 1,
            subscribers_count: 0,
            total_revenue: 0,
            commission_rate: 0.1, // 10% for tier 1
...
router.delete('/account', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      const { error: deleteUserError } = await userClient.from('users').delete().eq('id', userId);
...
router.post('/link-referral', authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    if (attributionError) {
      logger.error('Failed to store referral attribution', {
