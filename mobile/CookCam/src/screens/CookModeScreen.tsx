...
const getCookingTipsForRecipe = (
  numSteps: number): Array<{ emoji: string; tip: string }> => {
...
const CookModeScreen: React.FC<CookModeScreenProps> = ({
  ...
  const [_stepXPAnimations, setStepXPAnimations] = useState<number[]>([]);
  ...
  const initializeSteps = (recipeInstructions: unknown[]): CookingStep[] => {
  ...
  const handleRatingSubmit = async (ratingData: unknown) => {
