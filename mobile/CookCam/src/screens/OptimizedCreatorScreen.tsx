...
const OptimizedCreatorScreen: React.FC<CreatorScreenProps> = React.memo(({ navigation }) => {
  const { user } = useAuth();
  ...
  const progressToNext = calculateProgressToNext(currentTierData, nextTier, activeSubscribers);
  ...
  }, [analytics, progressToNext, fadeAnim, slideAnim, progressAnim, pulseAnim]);
    ...
    <View style={mixins.containers.screen}>
      ...
      >
        ...
        <View style={[mixins.containers.sectionPadded, { paddingBottom: 0 }]}>
          ...
          <Text style={[mixins.text.bodySecondary, { marginTop: tokens.spacing.sm }]}>
            Welcome back, Creator!
          </Text>
          {loading && (
            <View style={[mixins.layout.flexRow, { marginTop: tokens.spacing.xs }]}>
