...
type SubscriptionAction =
  ...
  | { type: 'SET_FEATURE_ACCESS'; feature: string; hasAccess: boolean; usage?: any }
...
function subscriptionReducer(state: SubscriptionState, action: SubscriptionAction): SubscriptionState {
  switch (action.type) {
    ...
    case 'SET_ERROR':
      return { ...state, subscriptionError: action.error, subscriptionLoading: false };
    ...
    case 'SET_SUBSCRIPTION':
      return { ...state, currentSubscription: action.subscription, lastChecked: Date.now() };
...
export function SubscriptionProvider({ children }: { children: ReactNode }) {
  ...
  const generateAffiliateLink = async (campaignName?: string): Promise<string> => {
    try {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      if (state.lastChecked && Date.now() - state.lastChecked > 10 * 60 * 1000) {
        refreshData();
      }
