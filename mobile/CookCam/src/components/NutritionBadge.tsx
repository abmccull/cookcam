...
const NutritionBadge: React.FC<NutritionBadgeProps> = React.memo(({
  ...
  const nutritionPerServing = useMemo(() => ({
    ...
    fiber: nutrition.fiber_g ? Math.round(nutrition.fiber_g / servings) : undefined,
    sodium: nutrition.sodium_mg ? Math.round(nutrition.sodium_mg / servings) : undefined,
  }), [nutrition, servings]);
  ...
  if (isCompact) {
    return (
      <View style={styles.compactContainer}>
        <View style={[styles.compactBadge, { backgroundColor: calorieLevel.color }]}>
          <Zap size={moderateScale(12)} color="#FFFFFF" />
          <Text style={styles.compactText}>
            {nutritionPerServing.calories}
          </Text>
        </View>
        <View style={[styles.compactBadge, { backgroundColor: proteinLevel.color }]}>
